...
/**
 * Implements the [Default] trait for the Bash configuration.
 */
...
/**
 * Unit tests for the Bash configuration.
 */
#[cfg(test)]
mod tests
{
    ...
    {
        let configuration: BashConfiguration = yaml_serde::from_str(r#"prompt:
  content: Test Prompt"#).expect("Failed to deserialise the partial configuration.");
    ...
    {
        let configuration = BashConfiguration::default();
        let deserialised: BashConfiguration = yaml_serde::from_str(&yaml_serde::to_string(&configuration).expect("Failed to serialise the base configuration.")).expect("Failed to deserialise the base configuration.");
