...
/**
 * Implements the [Default] trait for the Cmd configuration.
 */
...
/**
 * Unit tests for the Cmd configuration.
 */
#[cfg(test)]
mod tests
{
    ...
    {
        let configuration: CmdConfiguration = yaml_serde::from_str(r#"prompt:
  content: Test Prompt"#).expect("Failed to deserialise the partial configuration.");
    ...
    {
        let configuration = CmdConfiguration::default();
        let deserialised: CmdConfiguration = yaml_serde::from_str(&yaml_serde::to_string(&configuration).expect("Failed to serialise the base configuration.")).expect("Failed to deserialise the base configuration.");
