/*!
 * Stores the Git Bash configuration.
 */
...
/**
 * Implements the [Default] trait for the Git Bash configuration.
 */
...
/**
 * Unit tests for the Git Bash configuration.
 */
#[cfg(test)]
mod tests
{
    ...
    {
        let configuration: GitBashConfiguration = yaml_serde::from_str(r#"prompt:
  content: Test Prompt"#).expect("Failed to deserialise the partial configuration.");
    ...
    {
        let configuration = GitBashConfiguration::default();
        let deserialised: GitBashConfiguration = yaml_serde::from_str(&yaml_serde::to_string(&configuration).expect("Failed to serialise the base configuration.")).expect("Failed to deserialise the base configuration.");
