...
/**
 * Implements the [Default] trait for the Zsh configuration.
 */
...
/**
 * Unit tests for the Zsh configuration.
 */
#[cfg(test)]
mod tests
{
    ...
    {
        let configuration: ZshConfiguration = yaml_serde::from_str(r#"prompt:
  content: Test Prompt"#).expect("Failed to deserialise the partial configuration.");
    ...
    {
        let configuration = ZshConfiguration::default();
        let deserialised: ZshConfiguration = yaml_serde::from_str(&yaml_serde::to_string(&configuration).expect("Failed to serialise the base configuration.")).expect("Failed to deserialise the base configuration.");
